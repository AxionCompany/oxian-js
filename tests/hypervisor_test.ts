...
Deno.test({
  name: "hypervisor: basic proxy and streaming",
  sanitizeOps: false,
  sanitizeResources: false,
}, async () => {
  const port = 8140;
  const proc = new Deno.Command(Deno.execPath(), {
    args: ["run", "-A", "cli.ts", `--port=${port}`, "--hypervisor"],
    stdout: "piped",
    stderr: "piped",
  }).spawn();
  try {
    ...
    if (json.hello !== "world") {
      throw new Error("unexpected root response through HV");
    }
    res = await fetch(`http://localhost:${port}/stream`);
    const text = await res.text();
    if (!text.includes("hello") || !text.includes("world")) {
      throw new Error("unexpected stream body through HV");
    }
  } finally {
    try {
      proc.kill();
...
Deno.test({
  name: "hypervisor: sticky routing by header",
  sanitizeOps: false,
  sanitizeResources: false,
}, async () => {
  const port = 8142;
  ...
  try {
    ...
    try {
      const a = await fetch(`http://localhost:${port}/feature`, {
        headers: hdr,
        signal: ctl1.signal,
      });
      const b = await fetch(`http://localhost:${port}/feature`, {
        headers: hdr,
        signal: ctl2.signal,
      });
