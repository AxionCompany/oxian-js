...
async function startServer(port: number, configFile = "oxian.config.ts") {
  const proc = new Deno.Command("deno", {
    args: ["run", "-A", "cli.ts", `--port=${port}`, `--config=${configFile}`],
    stdout: "piped",
    stderr: "piped",
  }).spawn();
...
Deno.test("compat: handlerMode 'this' logs deprecation and returns value", async () => {
  ...
  await Deno.writeTextFile(
    "routes/compat_this/dependencies.ts",
    "export default ()=>({ val: 'ok-this' });\n",
  );
  await Deno.writeTextFile(
    "routes/compat_this/index.ts",
    "export function GET(d,{response}){ return (this && this.val) || 'none'; }\n",
  );
  await Deno.writeTextFile(
    "oxian.config.ts",
    "export default { compatibility: { handlerMode: 'this' } }\n",
  );
  const proc = await startServer(8138);
  try {
    const res = await fetch("http://localhost:8138/compat_this", {
    ...
    if (!text.includes("deprecation") || !text.includes("handlerMode 'this'")) {
      throw new Error("expected deprecation log for handlerMode 'this'");
    }
...
Deno.test("compat: handlerMode 'factory' logs deprecation and returns value", async () => {
  ...
  await Deno.writeTextFile(
    "routes/compat_factory/dependencies.ts",
    "export default ()=>({ val: 'ok-factory' });\n",
  );
  await Deno.writeTextFile(
    "routes/compat_factory/index.ts",
    "export default (deps)=> (d,{response})=> deps.val;\n",
  );
  await Deno.writeTextFile(
    "oxian.config.ts",
    "export default { compatibility: { handlerMode: 'factory' } }\n",
  );
  const proc = await startServer(8139);
  try {
    const res = await fetch("http://localhost:8139/compat_factory", {
      headers: { authorization: "Bearer x" },
    });
    const text = await res.text();
    ...
    const text = new TextDecoder().decode(out.stdout);
    if (
      !text.includes("deprecation") || !text.includes("handlerMode 'factory'")
    ) throw new Error("expected deprecation log for handlerMode 'factory'");
