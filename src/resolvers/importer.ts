...
const mapExtensionToWith = (
  extension?: string,
): "json" | "text" | "bytes" | undefined => {
  switch (extension) {
    case "json":
      return "json";
    case undefined:
...
export async function importModule(
  ...
  if (Deno.env.get("OXIAN_DEBUG") === "1") {
    try {
      console.log("importModule", sanitizeUrlForLog((url as URL).toString()), {
        ttl: _ttlMs,
      });
    } catch {
      console.log("importModule", sanitizeUrlForLog(String(url)), {
        ttl: _ttlMs,
      });
    }
  }
  ...
  try {
    ...
    if ((!urlStr.startsWith("file:"))) {
      const importDataUrl = `data:text/typescript;base64,${
        btoa(importModuleTemplate(urlStr, importType))
      }`;
      if (importType) {
    ...
    console.log("[importer] importModule error", e);
    throw e;
  }
}

const importModuleTemplate = (
  specifier: string,
  importType: "json" | "text" | "bytes" | undefined,
) => `
import * as mod from "${specifier}"${
  importType ? ` with { type: "${importType}" }` : ""
};
