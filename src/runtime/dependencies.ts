...
export async function composeDependencies(
  ...
  const resolveMod = async (url: URL): Promise<Record<string, unknown>> => {
    ...
    if (dependenciesFactoryCache.has(key)) {
      return dependenciesFactoryCache.get(key) as Record<string, unknown>;
    }
    try {
  ...
  const fileUrls = [
    ...
    ...(files.sharedFiles.length > 0 ? files.sharedFiles : []),
  ];
  ...
  for (const mod of modules) {
    ...
    const factory = (candidate.default ??
      (candidate as { dependencies?: unknown }).dependencies) as unknown;
