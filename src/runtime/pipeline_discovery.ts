...
export async function discoverPipelineFiles(
  ...
  if (DEBUG) {
    try {
      console.log("[pipeline] discover start", {
        levels: chain.map((l) => typeof l === "string" ? l : l.toString()),
      });
  ...
  for (const level of chain) {
    ...
    if (typeof level === "string") {
      ...
      if (level.protocol === "github:") {
        const basePath = level.pathname.endsWith("/")
          ? level.pathname
          : level.pathname + "/";
        const make = (name: string) =>
          new URL(`github:${(basePath + name).replace(/^\/+/, "")}`);
        ...
        baseObj.pathname = baseObj.pathname.endsWith("/")
          ? baseObj.pathname
          : baseObj.pathname + "/";
    ...
    promises.push(exists(depsjs));
    promises.push(exists(depsts));
    promises.push(exists(mwjs));
    promises.push(exists(mwts));
    promises.push(exists(icjs));
    promises.push(exists(icts));

    if (opts?.allowShared) {
      promises.push(exists(shjs));
      promises.push(exists(shts));
    }
    ...
    if (depstsOk && !depsjsOk) {
      files.dependencyFiles.push(
        typeof depsts === "string" ? toFileUrl(depsts) : depsts,
      );
    }
    if (mwjsOk) {
      files.middlewareFiles.push(
        typeof mwjs === "string" ? toFileUrl(mwjs) : mwjs,
      );
    }
    if (mwtsOk && !mwjsOk) {
      files.middlewareFiles.push(
        typeof mwts === "string" ? toFileUrl(mwts) : mwts,
      );
    }
    if (icjsOk) {
      files.interceptorFiles.push(
        typeof icjs === "string" ? toFileUrl(icjs) : icjs,
      );
    }
    if (ictsOk && !icjsOk) {
      files.interceptorFiles.push(
        typeof icts === "string" ? toFileUrl(icts) : icts,
      );
    }
    if (shjsOk) {
      files.sharedFiles.push(typeof shjs === "string" ? toFileUrl(shjs) : shjs);
    }
    if (shtsOk && !shjsOk) {
      files.sharedFiles.push(typeof shts === "string" ? toFileUrl(shts) : shts);
    }
  }
...
export async function buildLocalChain(
  resolver: Resolver,
  routesDir: string,
  routeFileUrl: URL,
): Promise<string[]> {
  const routesRoot = fromFileUrl(
    (await resolver.resolve(routesDir)).toString(),
  );
...
export async function buildRemoteChain(
  resolver: Resolver,
  routeFileUrl: URL,
): Promise<URL[]> {
  ...
  if (
    !routeFileUrl?.pathname?.split("/")?.filter(Boolean)?.join("/")?.startsWith(
      basePath,
    )
  ) return [];
  const rel = routeFileUrl.pathname?.split("/")?.filter(Boolean)?.join("/")
  ...
  let current = new URL(
    routesRootUrl.toString().endsWith("/")
      ? routesRootUrl
      : new URL(routesRootUrl.toString() + "/"),
  );
  // include routes root first
