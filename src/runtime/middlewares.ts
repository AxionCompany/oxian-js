...
function isMiddlewareObjectResult(
  input: unknown,
): input is { data?: Data; context?: Partial<Context> } {
...
const resolveMod = async (
  ...
  if (middlewaresCache.has(key)) {
    return middlewaresCache.get(key) as Record<string, unknown>;
  }
  ...
  for (const mod of modules) {
    ...
    if (typeof mw === "function") {
      if (config?.compatibility?.middlewareMode === "this") {
        ...
        const produced = (mw as (deps: unknown) => unknown)(
        ...
        mw = Object.assign(mw as (data: Data, context: Context) => unknown, {
          ...context.dependencies,
        });
      }
      ...
      if (config?.compatibility?.useMiddlewareRequest) {
        ...
        const res = await (mw as Middleware)(
          request as unknown as Data,
          currentContext,
        );
        if (
          res && typeof res === "object" &&
          "params" in (res as Record<string, unknown>)
        ) {
