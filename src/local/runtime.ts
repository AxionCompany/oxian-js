  ...
  const start = (): Promise<LocalRuntimeRunning> => {
    ...
    startTask = (async () => {
      try {
        ...
        const hypervisorReference: { current?: Hypervisor } = {};
        const gateway = createHttpGateway({
          dispatch: (input) => {
            const current = hypervisorReference.current;
            if (current === undefined) {
              return Promise.reject(
                new Error("local Hypervisor is not initialized"),
              );
            }
            return current.dispatch(input);
          },
        });
        ...
        hypervisorReference.current = hypervisor;
