...
import {
  ...
  type WorkerProvider,
} from "jsr:@oxian/oxian-js@0.21.0-rc.5/providers";
...
```sh
deno run -A jsr:@oxian/oxian-js@0.21.0-rc.5/bin worker --manifest oxian.worker.ts
deno run -A jsr:@oxian/oxian-js@0.21.0-rc.5/bin check --config oxian.config.ts
deno run -A jsr:@oxian/oxian-js@0.21.0-rc.5/bin routes --config oxian.config.ts
```
