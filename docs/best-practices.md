...
This guide consolidates proven patterns, architectural decisions, and operational practices for building robust, scalable, and maintainable APIs with Oxian. Learn from real-world experience to avoid common pitfalls and maximize your application's potential.
...
// utils/auth.ts
import { verify, sign } from "https://deno.land/x/djwt@v3.0.2/mod.ts";
...
export class AuthService {
  ...
  async createToken(user: { id: string; email: string; roles: string[] }): Promise<string> {
    const payload = {
      sub: user.id,
      email: user.email,
      roles: user.roles,
...
export class RateLimiter {
  ...
  check(request: Request): { allowed: boolean; remaining: number; resetTime: number } {
    ...
    if (!window || now > window.resetTime) {
      ...
      return { allowed: true, remaining: this.maxRequests - 1, resetTime: window.resetTime };
    }
...
export async function GET({ id }, { dependencies }) {
  ...
  try {
    ...
    logger.warn("Primary DB failed, trying cache", { error: primaryError.message });
    ...
    try {
      ...
      logger.warn("Cache failed, trying fallback API", { error: cacheError.message });
    }
...
Following these best practices will help you build robust, maintainable, and scalable APIs with Oxian. Start with the basics and gradually implement more advanced patterns as your application grows.
