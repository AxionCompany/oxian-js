...
if (import.meta.main) {
  ...
  if (cmd === "materialize") {
    try {
      ...
      const dir = typeof args["materialize-dir"] === "string"
  ...
  if (cmd === "routes") {
    if (!resolver) {
      ...
      resolver = createResolver(
        sourceStr ? new URL(sourceStr) : undefined,
        envDefaults,
      );
    }
  ...
  if (!bypassHv) {
    ...
    if (!resolver) {
      ...
      envDefaults2.tokenValue = envDefaults2.tokenEnv
        ? Deno.env.get(envDefaults2.tokenEnv)
        : undefined;
      resolver = createResolver(
        sourceStr ? new URL(sourceStr) : undefined,
        envDefaults2,
  ...
  if (!resolver) {
    ...
    envDefaults3.tokenValue = envDefaults3.tokenEnv
      ? Deno.env.get(envDefaults3.tokenEnv)
      : undefined;
    resolver = createResolver(
      sourceStr ? new URL(sourceStr) : undefined,
      envDefaults3,
